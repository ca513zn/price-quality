  ...
  if (submitted) {
    return (
      <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
        <p className="text-green-800 font-medium text-lg">✅ Vote submitted!</p>
        <p className="text-green-600 mt-1">Thanks for rating {productName}.</p>
        <button
          onClick={() => {
            setSubmitted(false);
            setPriceScore(5);
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 space-y-6">
      <h3 className="text-lg font-semibold text-gray-900">
        Rate <span className="text-blue-600">{productName}</span>
      </h3>

      {/* Price Score */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Perceived Price: <span className="text-blue-600 font-bold">{priceScore}</span>/10
        </label>
        <input
          type="range"
          min="1"
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Perceived Quality: <span className="text-emerald-600 font-bold">{qualityScore}</span>/10
        </label>
        <input
          type="range"
          min="1"
      ...
      <div className="text-center p-3 rounded-md bg-gray-50">
        <span className="text-sm text-gray-500">Your perception: </span>
        <span
